...
function mergeReactiveObjects<T extends StateTree>(
  ...
  for (const key in patchToApply) {
    ...
    if (
      ...
      // 要合并的和 store state 上的都是个Object 并且要合并的不是 ref 或者 不是 reactive 则递归合并
...
>(
  ...
  function setup() {
    ...
    // 合并 actions 和 getters 到 state 上
    return assign(
      localState,
      actions,
  ...
  // 定义reset方法用于重置 store 的状态到初始值
  store.$reset = function $reset() {
    const newState = state ? state() : {}
    // we use a patch to group all changes into one single subscription
    // 通过$patch 方法更新 store 的状态
    this.$patch(($state) => {
      // 通过方法 合并 store 的初始 state 和新的 state
...
>(
  ...
  function $patch(
    ...
    // 传入一个函数来对难以更改的状态进行设置 例如对数组进行添加等
  ...
  if (isVue2) {
    ...
    // 合并 state, getters, and action properties
 ...
 * Creates a `useStore` function that retrieves the store instance
 *
 * @param id - id of the store (must be unique) 必须唯一
 * @param storeSetup - function that defines the store 定义 store 的函数
